...
namespace QuestsExtended.Quests
{
    ...
    {
        private bool SniperCooldown = false;
        ...
        private float ArmourDamageholder;
        public void Awake()
        {
        ...
        //Damage
        public void EnemyDamageProcessor(DamageInfoStruct damage, float distance, string enemyID)
        {
            ...
            //Plugin.Log.LogInfo($"isScavRaid is set to {isScavRaid}");
            if (isScavRaid) return;
        ...
        public void ArmourDamageProcessor(float damage, DamageInfoStruct damageInfo, Player enemy)
        {
            if (isScavRaid) return;
            if (damage > 0f)
            {
        ...
        public void BodyPartDestroyed(DamageInfoStruct damageInfo, EBodyPart bodyPart, Player enemy)
        {
            if (isScavRaid) return;
        ...
        public void EnemyKillProcessor(Player attackingPlayer, DamageInfoStruct damageInfo, string enemyID)
        {
            if (isScavRaid) return;
            QuestExtendedController qeController = null;
            foreach (QuestExtendedController QEC in Singleton<GameWorld>.Instance.gameObject.GetComponents<QuestExtendedController>())
            {
                if (attackingPlayer == QEC._player)
                {
                    qeController = QEC;
                }
            }
            if (qeController == null) { Plugin.Log.LogWarning("Failed to get QEC on EnemyKillProcessor. Aborting"); return; }
            ...
            if (qeController._physicalController.isCrouched)
            {
            ...
            else if (qeController._physicalController.isProne)
            {
            ...
            if (qeController._physicalController._movementContext.IsInMountedState)
            {
            ...
            if (qeController._physicalController.isSilent)
            {
            ...
            if (qeController._physicalController.isADS)
            {
            ...
            if (qeController._physicalController.isBlindFiring)
            {
        ...
        public void MalfunctionFixed(Weapon weapon)
        {
            if (isScavRaid) return;
            ...
            Plugin.Log.LogInfo("Malfunction was fixed.");
        }
        //Looting
        private bool LootingXPCooldown;
        public void UniqueItemLooted()
        {
            if (isScavRaid) return;
        ...
        private IEnumerator LootingCooldown()
        {
        ...
        private HashSet<string> SearchedContainers = new HashSet<string>();

        public void SearchingContainer (Item item)
        {
            if (isScavRaid) return;
        ...
        private bool SwitchCooldown; //The switch method that BSG uses likes to call itself quite a few times... let's ensure it's only called once.
        public void PowerSwitchInteractedWith()
        {
            if (isScavRaid) return;
        ...
        //Using keys
        public void PlayerUsedKeyToUnlockDoor(string keyId)
        {
            if (isScavRaid) return;
        ...
        private IEnumerator SwitchCooldownTimer()
        {
            SwitchCooldown = true;
        ...
        private IEnumerator StartSniperCooldown()
        {
            yield return new WaitForSeconds(0.2f);
            SniperCooldown = false;
        }

        private bool CheckForCorrectEnemyType(Player enemy, ConditionPair cond)
        {
